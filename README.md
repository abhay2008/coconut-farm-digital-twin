...
- **📊 Smart Agriculture Telemetry Suite**: Interactive calculators for Hargreaves Evapotranspiration ($ET_0$), Penman Open-Water Pond Solar Evaporation (Liters lost per day from 500kL pond), Venturi N-P-K fertigation liquid injection rates ($\text{L/hr}$), Total Dynamic Head ($\text{TDH}$ in meters), and AP Transco 100% Free Power Policy tracking ($\mathbf{\text{₹}0\text{ Tariff}}$ for AP farmers).
